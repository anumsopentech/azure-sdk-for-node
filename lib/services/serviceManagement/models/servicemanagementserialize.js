...
* @param {string} deploymentName    The name of the deployment.
* @param {string} roleName          The name of the role.
* @param {bool}   stayProvisioned   Whether to keep the VM in provisioned state or not.
* @param {object} client            The servicemanagement object.
*/
ServiceManagementSerialize.prototype.buildShutdownRole = function (serviceName, deploymentName, stayProvisioned,
                                                    roleName, client) {
  if (client.serializetype === 'XML') {
    var doc = _createXmlRoot('ShutdownRoleOperation');
    doc.ele('OperationType').txt('ShutdownRoleOperation');
    if (!stayProvisioned) {
      doc.ele('PostShutdownAction').txt('StoppedDeallocated');
    }
